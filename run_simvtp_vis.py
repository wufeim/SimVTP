...
import utils
import modeling_pretrain_vis
from build_datasets import DataAugmentationForSimVTP
...
class DataAugmentationForSimVTP(object):
    ...
    def __repr__(self):
        repr = "(DataAugmentationForSimVTP,\n"
        repr += "  transform = %s,\n" % str(self.transform)
...
def get_args():
    parser = argparse.ArgumentParser('SimVTP visualization reconstruction script', add_help=False)
    ...
    # Model parameters
    parser.add_argument('--model', default='pretrain_simvtp_base_patch16_224', type=str, metavar='MODEL',
...
def main(args):
    ...
    transforms = DataAugmentationForSimVTP(args)
    ...
    with torch.no_grad():
        ...
        outputs, mlm_logits , _, _, _, _ = model(img, bool_masked_pos, text_token)
        ...
        text_index = [ str(i) for i in range(len(data))]
        vocab = dict(zip(text_index,data))
        
        word_index[mlm_labels[0] == -100] = input_ids_no_mlm[0][mlm_labels[0] == -100]
        
        pre_text_str = ''
        for i in range(len(word_index)):
            pre_text_str = pre_text_str + ' ' + vocab[str(word_index[i].item())]
        
        mask_text_index = torch.zeros_like(input_ids_no_mlm[0]).to(device)
        mask_text_index[mlm_labels[0] == -100] = input_ids_no_mlm[0][mlm_labels[0] == -100]
        mask_text_str = ''
        for i in range(len(mask_text_index)):
            if mask_text_index[i] == 0:
                mask_text_str = mask_text_str + ' [mask] '
            else:
                mask_text_str = mask_text_str + ' ' + vocab[str(mask_text_index[i].item())]
        print('#========================================================================================================================================================')
        print('orign_text_str: ', text_str)
        print('#========================================================================================================================================================')
        print('mask_text_str: ', mask_text_str)
        print('#========================================================================================================================================================')
        print('pre_text_str : ',pre_text_str)
