...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, mask, text_embedding, text_attention_mask, N_vis=0):
        _, _, T, _, _ = x.shape
    ...
    def forward(self, x, mask, text_embedding, text_attention_mask):
        x = self.forward_features(x, mask, text_embedding, text_attention_mask)
        x = self.head(x)
        return x
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask, text_data):
        ...
        x_vis_vt = self.encoder(x, mask, text_embedding, text_attention_mask) # [B, N_vis, C_e]
        ...
        return  x_full, mlm_logits, _, _, _, _
